...
fn parse_into_map<F,K,V,R>(rdr:R, f:F) -> HashMap<K,V>
    where F:Fn(String) -> (K,V),
          K:Hash+Eq,
          R:BufRead,{
    rdr.lines()
        .filter_map(|mline| match mline {
...
fn main() {
    ...
    let posting = parse_into_map(BufReader::new(posting_in), parse_posting);
